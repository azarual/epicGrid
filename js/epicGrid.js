...
function epicGrid() {
    ...
    for (d = 0; d < b.length; ++d)
        for (var c = b[d].children, f = e = 0; f < c.length; ++f) {
            ...
            if (!new RegExp(k + "-col-\\d+-\\d+", "g").test(c[f].className) || g.test(c[f].className) || h.test(c[f].className) || (k = c[f].className.match(m).toString().match(/\d+/g), 1 <= Math.round(1e4 * e) / 1e4 && (e += -1), 0 == Math.round(1e4 * e) / 1e4 && (c[f].className += " eg-first"), e = 2 < k.length ? e + (k[0] / k[1] + k[2] / k[3]) : e + k[0] / k[1], 0 == Math.round(1e4 * e) / 1e4 % 1 && (k = document.createElement("div"), k.style.clear = "both", k.className = "egcf", c[f].parentNode.insertBefore(k, c[f].nextSibling))), n.test(c[f].className))
    ...
    a = new CustomEvent("eg-ready"), document.dispatchEvent(a)
}
